...
void to_cognitec_image(janus_media media, vector<Image> &cognitec_images)
{
...
{
	vector<Image> cognitec_images;
	to_cognitec_image(media,cognitec_images);

	for (size_t i = 0; i< cognitec_images.size();i++)
	{
		//Find interpupillary markers in the image
		Face::LocationSet locations = face_detector->find(cognitec_images[i]);
        ...
        while( faceIter != locations.end())
        {
        		...
        		Eyes::LocationSet eye_locations = eye_detector->find(cognitec_images[i],*faceIter);
			...
			AnnotatedImage annotated (cognitec_images[i],eye_location);
