...
static janus_image janus_from_pittpatt(ppr_raw_image_type *ppr_image)
{
    ...
    image.data = malloc(image.width * image.height * (image.color_space == JANUS_BGR24 ? 3 : 1));
    const janus_size elements_per_row = image.width * (image.color_space == JANUS_BGR24 ? 3 : 1);
