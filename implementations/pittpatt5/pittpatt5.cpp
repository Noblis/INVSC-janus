...
uint8_t *crop_frame(janus_media_type &media,
                    int &x, int &y, int &w, int &h) // IN (requested ROI) /OUT (actual ROI)
{
    cv::Mat tmp;
    ...
    x = x < 0 ? 0 : x;
    y = y < 0 ? 0 : y;
    w = (x + w > (int) media.width ) ? media.width  - x : w;
    h = (y + h > (int) media.height) ? media.height - y : h;
    cv::Rect roi(x, y, w, h);
                          ...
                          vector<janus_cluster_item> &clusters)
{
    ...
    if (clustering_aggressiveness > (int) hint)
